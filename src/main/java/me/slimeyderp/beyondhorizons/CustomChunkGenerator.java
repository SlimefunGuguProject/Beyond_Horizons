...
public class CustomChunkGenerator extends ChunkGenerator {
    ...
    int NewcurrentHeight = 120;
    Random rand = new Random();

    @Override
    public ChunkData generateChunkData(World world, Random random, int chunkX, int chunkZ, BiomeGrid biome) {
        SimplexOctaveGenerator generator = new SimplexOctaveGenerator(new Random(world.getSeed()), 8);
        SimplexOctaveGenerator secondgenerator = new SimplexOctaveGenerator(new Random(world.getSeed()), 8);
        ChunkData chunk = createChunkData(world);
        generator.setScale(0.05D);
        secondgenerator.setScale(0.065D);

        for (int X = 0; X < 16; X++)
            for (int Z = 0; Z < 16; Z++) {
                currentHeight = (int) (generator.noise(chunkX * 16 + X, chunkZ * 16 + Z, 0.5D, 0.5D) * 15D + 50D);
                NewcurrentHeight = (int) (secondgenerator.noise(chunkX * 16 + X, chunkZ * 16 + Z, 0.5D, 0.5D) * 15D + 50D);
                int randomNumber1 = rand.nextInt(11);
                if (randomNumber1 > 8) {
                    chunk.setBlock(X, currentHeight, Z, Material.MAGENTA_TERRACOTTA);
                    chunk.setBlock(X, NewcurrentHeight + 60, Z, Material.MAGENTA_TERRACOTTA);
                } else {
                    chunk.setBlock(X, currentHeight, Z, Material.PURPLE_GLAZED_TERRACOTTA);
                    chunk.setBlock(X, NewcurrentHeight + 60, Z, Material.PURPLE_GLAZED_TERRACOTTA);
                }
                int randomNumber2 = rand.nextInt(11);
                if (randomNumber2 < 4) {
                    chunk.setBlock(X, currentHeight - 1, Z, Material.PURPUR_BLOCK);
                    chunk.setBlock(X, NewcurrentHeight + 61, Z, Material.PURPUR_BLOCK);
                } else {
                    chunk.setBlock(X, currentHeight - 1, Z, Material.PURPUR_PILLAR);
                    chunk.setBlock(X, NewcurrentHeight + 61, Z, Material.PURPUR_PILLAR);
                }
                for (int i = currentHeight - 2; i > 0; i--) {
                    chunk.setBlock(X, i, Z, Material.END_STONE);
                }
                for (int h = NewcurrentHeight + 62; h < 255; h++) {
                    chunk.setBlock(X, h, Z, Material.END_STONE);
                }
